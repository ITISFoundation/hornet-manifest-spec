...
def handle_service_exceptions(operation_name: str = "operation"):
    ...
    Note: This is the legacy sync version, kept for backward compatibility.
    New code should use handle_service_exceptions_async.
    """
...
def handle_service_exceptions_async(operation_name: str = "operation"):
    """Async decorator to handle common service layer exceptions and convert them to API exceptions."""
...
class WorkflowAPI:
    ...
    @handle_service_exceptions_async("workflow operation")
    async def run(
        self,
    ...
    @handle_service_exceptions_async("watch operation")
    async def watch(
        self,
        inputs_dir: str | Path,
        work_dir: str | Path,
        ...
        await watcher.watch_for_metadata(
...
class RepoAPI:
    ...
    @handle_service_exceptions_async("clone repository")
    async def clone(
        ...
        repo_path = await git_service.clone_repository(repo_url, commit, dest_path)
        return repo_path
...
class ManifestAPI:
    ...
    async def validate_schema(self, manifest_path: Path, manifest_type: str) -> None:
        """Validate a manifest schema."""
        try:
            await manifest_service.validate_manifest_schema(manifest_path)
        except jsonschema.ValidationError as e:
            msg = f"{manifest_type} manifest schema validation failed: {e.message}"
            raise ApiValidationError(msg) from e

    @handle_service_exceptions_async("manifest validation")
    async def validate(self, repo_path: Path) -> tuple[bool, bool]:
        """Validate hornet manifests against their schemas."""
        repo_dir = Path(repo_path)
        cad_manifest, sim_manifest = await manifest_service.find_hornet_manifests(
            repo_dir
        )
        ...
        if cad_manifest:
            with contextlib.suppress(ApiValidationError):
                await self.validate_schema(cad_manifest, "CAD")
                cad_valid = True

        if sim_manifest:
            with contextlib.suppress(ApiValidationError):
                await self.validate_schema(sim_manifest, "SIM")
                sim_valid = True

        return cad_valid, sim_valid

    @handle_service_exceptions_async("manifest show")
    async def show(self, repo_path: str, manifest_type: str = "both") -> dict[str, Any]:
        ...
        if manifest_type.lower() in ["cad", "both"] and cad_manifest:
            result["cad"] = await manifest_service.read_manifest_contents(cad_manifest)

        # Get SIM manifest if requested and exists
        if manifest_type.lower() in ["sim", "both"] and sim_manifest:
            result["sim"] = await manifest_service.read_manifest_contents(sim_manifest)

        return result

    @handle_service_exceptions_async("manifest processing")
    async def process_with_plugin(
        self,
        cad_manifest: Path,
        repo_path: Path,
    ...
    async def process(
        ...
        if cad_manifest:
            return await self.process_with_plugin(
                cad_manifest,
                repo_path,
                plugin_name,
                type_filter,
...
class CadAPI:
    ...
    @handle_service_exceptions_async("CAD loading")
    async def load(
        self,
        repo_path: str,
        plugin: str | None = None,
...
class HornetFlowAPI:
    ...
    async def info(self) -> dict[str, str | bool | dict[str, Any]]:
        ...
        """
        # Get Git version
        git_version = await git_service.check_git_version()
