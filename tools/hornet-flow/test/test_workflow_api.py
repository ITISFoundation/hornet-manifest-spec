...
def test_workflow_run_basic(mocker: MockerFixture, api: HornetFlowAPI) -> None:
    ...
    assert call_args.kwargs['repo_url'] == "https://github.com/COSMIIC-Inc/Implantables-Electrodes"
    assert call_args.kwargs['plugin'] == "osparc"
    assert call_args.kwargs['fail_fast'] is True


def test_workflow_watch_basic(mocker: MockerFixture, api: HornetFlowAPI, tmp_path: Path) -> None:
...
def test_workflow_watch_nonexistent_directory(api: HornetFlowAPI) -> None:
    ...
    with pytest.raises(ApiFileNotFoundError):
        api.workflow.watch(
            inputs_dir="/nonexistent/directory",
            work_dir="/tmp/work"
        )
...
def test_workflow_run_with_event_dispatcher(mocker: MockerFixture, api: HornetFlowAPI) -> None:
...
def test_watch_with_event_dispatcher(mocker: MockerFixture, api: HornetFlowAPI, tmp_path: Path) -> None:
